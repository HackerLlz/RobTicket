...
(function() {
	$.stopStation = function(a) {
		...
		b.open = function(f, j, h, e, i, c) {
			...
			var stationInfoUrl = '/ticket/stationInfo';
			var stationInfoData = {
                train_no: j,
			...
			$.ajax({
				...
				// 	depart_date: g
				// },
				data: JSON.stringify(stationInfoData),
...
(function() {
	...
	var bq = function(cy) {
		...
		this.init = function() {
			...
			cA.css(cy, ct.x + "px").css({
				top: ct.y + "px"
			}).on("mouseenter", cw.stop).on("mouseleave", cw.resize).children("a.close").on("click", cw.hidden);
			$("body").append(cA);
			cs = ct.x;
			cx = ct.y;
			cw.resize()
	...
	function aj(cr) {
		...
		// cr就是queryParams
        var queryData = {
            "leftTicketDTO.train_date": cr["trainDate"],
	...
	function ck(cs) {
		var cr;
		if ("X" == canChooseBeds) {
			$("#bed_show").html("<span style='background:url(../../images/icon_new.png) right center no-repeat; padding-right:30px; cursor: pointer;' title='欢迎使用12306选铺功能'>铺别</span>");
	...
	function R() {
		...
		ac = function(cB, cy, cv, cu, cx) {
			...
			var cw = "<P>" + cv + "</p>";
			var cz = cy ? '<p>请点击[<a href="' + ctx + 'view/train_order.html?type=2"><strong>我的12306</strong></a>]办理其他业务。您也可以点击[<a href="' + ctx + 'leftTicket/init"><strong>预订车票</strong></a>]，重新规划您的旅程。</p>' : '<P>查看订单处理情况，请点击“<a href="https://kyfw.12306.cn/otn/view/train_order.html?type=1" target="_blank">未完成订单</a>”</P>';
	...
	showTicketPrice = function(cv) {
		...
		if (ct && ($("#ticket_" + cw + "_train>span>span").text() == "查看票价")) {
			...
			var ticketPriceUrl = '/ticket/ticketPrice';
			var ticketPriceData = {
                train_no: cw,
                from_station_no: cy,
                to_station_no: cu,
            ...
            var ticketPriceJson1 = {
                "data": ticketPriceData,
                "url": "leftTicket/queryTicketPriceFL"
            };
            var ticketPriceJson2 = {
                "data": ticketPriceData,
                "url": "leftTicket/queryTicketPrice"
            };
			$.ajax({
				// type: "get",
                type: "post",
                ...
                url: ticketPriceUrl,
                data: JSON.stringify(ticketPriceJson1),
                contentType:'application/json',    // 不加传过去的json后面有个= 会出问题
                dataType: "json",    // 加了data就不用再转json对象了
			...
			$.ajax({
                ...
                url: ticketPriceUrl,
                data: JSON.stringify(ticketPriceJson2),
                contentType:'application/json',    // 不加传过去的json后面有个= 会出问题
                dataType: "json",    // 加了data就不用再转json对象了
	...
	function aX(cs, cr) {
	    ...
	    var redirectUrl = '/passenger/view';
	    // js组装表单实现带数据跳转
        // otsRedirect("post", redirectUrl, redirectData);
        // otsRedirect("post", redirectUrl, {});
		if (cr != null) {
            if (cr == "fc") {
                // otsRedirect("post", ctx + "confirmPassenger/initFc", {});
                otsRedirect("post", redirectUrl, {url: "confirmPassenger/initFc"});
                return
            }
            if (cr == "gc") {
                // otsRedirect("post", ctx + "confirmPassenger/initGc", {});
                otsRedirect("post", redirectUrl, {url: "confirmPassenger/initGc"});
                return
            }
            if (cs == "dc") {
                // otsRedirect("post", ctx + "confirmPassenger/initDc", {});
                otsRedirect("post", redirectUrl, {url: "confirmPassenger/initDc"});
                return
            } else {
                // otsRedirect("post", ctx + "confirmPassenger/initWc", {})
                otsRedirect("post", redirectUrl, {url: "confirmPassenger/initWc"});
            }
        }
	}
...
function validateSecIdCard(g) {
	...
	for (var b = 17; b >= 0; b--) {
		f += (Math.pow(2, b)) * parseInt(a.charAt(17 - b), 11)
	}
	if (f!= 1) {
		return false
	}
...
jQuery.extend({
	...
	popup_uamIsLogin: function() {
		...
		$.ajax({
			...
			// },
            url: uamtkStaticUrl,
            data: JSON.stringify(uamtkStaticData),
            contentType:'application/json',
            dataType: 'json',
			type: "POST",
			// xhrFields: {
	...
	popup_checkPassCode: function() {
        ...
        var checkCodeUrl = '/login/checkCode';
		$.ajax({
